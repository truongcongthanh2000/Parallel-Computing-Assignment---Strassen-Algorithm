...
void load(lld** &matA, lld** &matB, int &n, int &m, int &p) {
    fstream finp;
    ...
    for (int i = 0; i < n; i++) {
        matA[i] = new lld[m];
        for (int j = 0; j < m; j++) finp >> matA[i][j];
    }
    finp >> m >> p;
    matB = new lld*[m];
    for (int i = 0; i < m; i++) {
        matB[i] = new lld[p];
        for (int j = 0; j < p; j++) finp >> matB[i][j];
    }
    finp.close();
}
int main()
{
    ...
    lld** matA;
    lld** matB;
    load(matA, matB, n, m, p);
    clock_t begin, end;
    begin = clock();

    lld** matC = Strassen(matA, matB, n, m, p);
