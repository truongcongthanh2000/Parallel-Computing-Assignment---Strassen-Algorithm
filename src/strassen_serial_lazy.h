...
private:
    Matrix<T> strassen(array<Matrix<T>, 2>& mats) const {
        ...
        transform(dimensions_new.begin(), dimensions_new.end(), dimensions_sub.begin(), [](size_t x) { return x >> 1; });  // this is dimensions / 2
        ...
        res_sub[0][1].resize(dimensions_sub[0], dimensions[1] >> 1);
        res_sub[1][1].resize(dimensions[0] >> 1, dimensions[1] >> 1);
        res_sub[1][0].resize(dimensions[0] >> 1, dimensions_sub[1]);
    ...
    bool reached_base_dimensions(const std::array<size_t, 3>& dimensions) const {
        ...
        return all_of(dimensions.cbegin(), dimensions.cend(), [](size_t x) { return x <= B; });
    }
};
