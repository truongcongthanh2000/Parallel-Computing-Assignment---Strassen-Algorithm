...
class StrassenOpenMpEager : public Decorator<T> {
public:
...
private:
    Matrix<T> strassen(array<Matrix<T>, 2>& mats) const {
        ...
        s.at(0).assign_parallel(mats_sub.at(0)[0][0]).add_parallel(mats_sub.at(0)[1][1]);  // a00 + a11
        s.at(1).assign_parallel(mats_sub.at(1)[0][0]).add_parallel(mats_sub.at(1)[1][1]);  // b00 + b11
        p.at(0) = this->strassen(s);                               // p0 = (a00 + a11) * (b00 + b11)

        for (size_t i = 0; i < 2; ++i) {
            s.at(0).assign_parallel(mats_sub.at(0)[1 - i][i]).add_parallel(mats_sub.at(0)[1 - i][1 - i]);  // a10 + a11                    a01 + a00
        ...
        for (size_t i = 0; i < 2; ++i) {
            ...
            s.at(1).assign_parallel(mats_sub.at(1)[i][1 - i]).sub_parallel(mats_sub.at(1)[1 - i][1 - i]);  // b01 - b11                    b10 - b00
            p.at(3 + i) = this->strassen(s);                                       // p3 = a00 * (b01 - b11)       p4 = a11 * (b10 - b00)
            s.at(0).swap(mats_sub.at(0)[i][i]);
        }

        for (size_t i = 0; i < 2; ++i) {
            s.at(0).assign_parallel(mats_sub.at(0)[1 - i][i]).sub_parallel(mats_sub.at(0)[i][i]);      // a10 - a00                            a01 - a11
            s.at(1).assign_parallel(mats_sub.at(1)[i][i]).add_parallel(mats_sub.at(1)[i][1 - i]);      // b00 + b01                            b11 + b10
        ...
        for (size_t i = 0; i < 2; ++i) {
            res_sub.at(i).at(i).assign_parallel(p.at(0)).sub_parallel(p.at(2 - i)).add_parallel(p.at(4 - i)).add_parallel(p.at(6 - i));   // c00 = p0 - p2 + p4 + p6      c11 = p0 - p1 + p3 + p5
            res_sub.at(i).at(1 - i).assign_parallel(p.at(2 - i)).add_parallel(p.at(3 + i));                       // c01 = p2 + p3                c10 = p1 + p4
        }
